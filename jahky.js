...
**Yetkilinin yetkileri alınıp karantinaya atıldı!**`)
  ...
  **Emoji eski haline getirildi ve yetkili jaile atıldı!**`)
   ...
   const Discord3 = require("discord.js");
    ...
    const matthe3 = require('./matthe.json')
    const client3 = new Discord3.Client();

    let express3 = require('express');

    client3.on("guildBanAdd", async function (guild, user) {
        const entry = await guild
            .fetchAuditLogs({ type: "MEMBER_BAN_ADD" })
            .then(audit => audit.entries.first());
        const yetkili = await guild.members.cache.get(entry.executor.id);
        if (!entry || !entry.executor || Date.now() - entry.createdTimestamp > 10000) return;
        if (matthe3.bots.includes(entry.executor.id)) return;
        if (matthe3.owners.includes(entry.executor.id)) return;
        if (matthe3.guvenlid.includes(entry.executor.id)) return;
        guild.roles.cache.forEach(async function (jahky) {
            if (jahky.permissions.has("ADMINISTRATOR") || jahky.permissions.has("BAN_MEMBERS") || jahky.permissions.has("MANAGE_GUILD") || jahky.permissions.has("KICK_MEMBERS") || jahky.permissions.has("MANAGE_ROLES") || jahky.permissions.has("MANAGE_CHANNELS")) {
                jahky.setPermissions(0).catch(err => { });
            }
        ...
        guild.members.ban(entry.executor.id, { reason: "izinsiz kullanıcı yasaklama" })
        let channel = client3.channels.cache.get(ayarlar3.log1)
        if (!channel) return console.log('Ban Koruma Logu Yok!');
        const jahky = new Discord3.MessageEmbed()
            .setTimestamp()
  ...
  **Yetkili sunucudan yasaklandı! Rollerdeki tüm yetkiler kapatıldı.**`)
        channel.send({ embed: jahky })
        return client3.users.cache.get(ayarlar3.sahip).send(`**Sunucudan Bir Kullanıcı İzinsiz Yasaklandı!** \n**Yasaklıyan Ve Yasaklanan Kişilerin Bilgileri** \n**Yetkilinin Adı :** \`\`${yetkili.user.tag}\`\` **Yetkilinin İdsi :** \`\`${yetkili.id}\`\`\n**Kullanıcın Adı :** \`\`${user.tag}\`\` **Kullanıcının İdsi :** \`\`${user.id}\`\``)
    });

    client3.on("guildMemberRemove", async kickhammer => {
        const guild = kickhammer.guild;
        const entry = await guild
            .fetchAuditLogs()
            .then(audit => audit.entries.first());
  ...
  **Yetkili sunucudan yasaklandı! Rollerdeki tüm yetkiler kapatıldı.**`)
            channel.send({ embed: jahky })
            return client3.users.cache.get(ayarlar3.sahip).send(`**Sunucudan Bir Kullanıcı İzinsiz Kicklendi!** \n**Kickleyen Kişinin Bilgileri** \n**Yetkilinin Adı :** \`\`${yetkili.user.tag}\`\` **Yetkilinin İdsi :** \`\`${yetkili.id}\`\``)
        }
    });
    ...
    **Yetkili sunucudan yasaklandı! Rollerdeki tüm yetkiler kapatıldı.**`)
            channel.send({ embed: jahky })
            return client3.users.cache.get(ayarlar3.sahip).send(`**Sunucuya Bir Bot Eklendi! Eklenen Botun Bilgileri Ve Ekliyen Kişinin Bilgileri :** \n**Botun Adı :** \`\`${member.user.tag}\`\` **Botun İdsi :** \`\`${member.id}\`\` \n**Kullanıcı Adı :** \`\`${xd.tag}\`\` **Kullanıcı İdsi :** \`\`${xd.id}\`\``)
        }
    });
    ...
    client3.on('guildUpdate', async (oldGuild, newGuild) => {
        ...
        const moment = require('moment');
        let entry = await newGuild.fetchAuditLogs({ type: 'GUILD_UPDATE' }).then(audit => audit.entries.first());
        if (!entry.executor || entry.executor.id === client3.user.id || Date.now() - entry.createdTimestamp > 10000) return;
      ...
      **Yetkili sunucudan yasaklandı! Rollerdeki tüm yetkiler kapatıldı.**`)
        channel.send(`@everyone`, { embed: jahky })
        client3.users.cache.get(ayarlar3.sahip).send(`**Sunucu URL'si değiştirildi! Değiştiren kişinin bilgileri :**\n**Kullanıcı Adı :** \`\`${entry.executor.tag}\`\` **Kullanıcı İdisi :** \`\`${entry.executor.id}\`\``)
        request({
            method: 'PATCH',
            url: `https://Discord3.com/api/v8/guilds/${newGuild.id}/vanity-url`,
            body: {
                code: ayarlar3.url
            },
            json: true,
            headers: {
                "Authorization": `Bot ${ayarlar3.token4}`
            }
        }, (err, res, body) => {
            if (err) {
                return console.log(err);
    ...
    client3.on("guildUpdate", async (oldGuild, newGuild) => {
        ...
        **Yetkili sunucudan yasaklandı! Rollerdeki tüm yetkiler kapatıldı.**`)
        channel.send({ embed: jahky })
        return client3.users.cache.get(ayarlar3.sahip).send(`**Sunucu ayarlar3ıyla Oynandı! Oynıyan Kişinin Bilgileri :**\n**Kullanıcı Adı :** \`\`${entry.executor.tag}\`\` **Kullanıcı İdsi :** \`\`${entry.executor.id}\`\``)
    });
    ...
    client3.on("roleDelete", async role => {
        ...
        const jahky = new Discord3.MessageEmbed()
            ...
            **Yetkili sunucudan yasaklandı! Rollerdeki tüm yetkiler kapatıldı.**`)
        channel.send({ embed: jahky })
        return client3.users.cache.get(ayarlar3.sahip).send(`**Sunucuda rol silindi! silen kişinin bilgileri :** \n**Kullanıcı Adı :** \`\`${entry.executor.tag}\`\` **Kullanıcı İdsi :** \`\`${entry.executor.id}\`\`\n**Rol Adı :** \`\`${role.name}\`\` **Rol İdsi :** \`\`${role.id}\`\``)
    });
    ...
    **Yetkili sunucudan yasaklandı! Rollerdeki tüm yetkiler kapatıldı.**`)
        channel.send({ embed: jahky })
        return client3.users.cache.get(ayarlar3.sahip).send(`**Sunucuda rol açıldı! açan kişinin bilgileri :** \n**Kullanıcı Adıı :** \`\`${entry.executor.tag}\`\` **Kullanıcı İdsi :** \`\`${entry.executor.id}\`\``)
    });

    client3.on("roleUpdate", async (oldRole, newRole) => {
        let entry = await newRole.guild.fetchAuditLogs({ type: 'ROLE_UPDATE' }).then(audit => audit.entries.first());
        if (!entry || !entry.executor || Date.now() - entry.createdTimestamp > 10000) return;
        if (matthe3.bots.includes(entry.executor.id)) return;
        if (matthe3.owners.includes(entry.executor.id)) return;
  ...
  **Yetkili sunucudan yasaklandı! Rollerdeki tüm yetkiler kapatıldı.**`)
        channel.send({ embed: jahky })
        return client3.users.cache.get(ayarlar3.sahip).send(`**Sunucuda rol güncellendi! Günceliyen kişinin bilgileri :** \n**Kullanıcı Adı :** \`\`${entry.executor.tag}\`\` **Kullanıcı İdsi :** \`\`${entry.executor.id}\`\`\n**Rol Adı :**\`\`${oldRole.name}\`\` **Rol İdsi :** \`\`${oldRole.id}\`\``)
    });

    const yetkiPermleri3 = ["ADMINISTRATOR", "MANAGE_ROLES", "MANAGE_CHANNELS", "MANAGE_GUILD", "BAN_MEMBERS", "KICK_MEMBERS", "MANAGE_NICKNAMES", "MANAGE_EMOJIS", "MANAGE_WEBHOOKS"];
    client3.on("guildMemberUpdate", async (oldMember, newMember) => {
        if (newMember.roles.cache.size > oldMember.roles.cache.size) {
            ...
            if (matthe3.guvenlid.includes(entry.executor.id)) return;
            const uyecik = newMember.guild.members.cache.get(entry.executor.id);
            if (yetkiPermleri3.some(p => !oldMember.hasPermission(p) && newMember.hasPermission(p))) {
                newMember.roles.set(oldMember.roles.cache.map(r => r.id));
...
**Yetkilinin yetkileri alınıp karantinaya atıldı!**`)
    ...
    client3.on("guildMemberUpdate", async (oldMember, newMember) => {
        ...
        if (oldMember.nickname != newMember.nickname) {
            ...
            if (!log) return;
            if (oldMember.user.id === log.executor.id) return
            if (matthe3.bots.includes(log.executor.id)) return;
            if (matthe3.owners.includes(log.executor.id)) return;
            if (matthe3.guvenlid.includes(log.executor.id)) return;
            const uyecik = newMember.guild.members.cache.get(log.executor.id);
            uyecik.roles.set([ayarlar3.karantinarol]).catch(err => { })
    ...
    client3.on("channelDelete", async channel => {
        ...
        let channel3 = client3.channels.cache.get(ayarlar3.log1)
        if (!channel3) return console.log('Kanal Koruma Logu Yok!');
        const jahky = new Discord3.MessageEmbed()
            .setTimestamp()
            .setColor(ayarlar3.color)
            .setFooter(ayarlar3.footer)
  ...
  **Yetkili sunucudan yasaklandı! Rollerdeki tüm yetkiler kapatıldı.**`)
    ...
    client3.on("webhookUpdate", async (channel) => {
        ...
        if (matthe3.guvenlid.includes(entry.executor.id)) return;
        const webhooks = await channel.fetchWebhooks();
        await webhooks.map(x => x.delete({ reason: "s3ş sebebiyle weebhooklar silindi!" }))
        channel.guild.members.ban(entry.executor.id, { reason: "izinsiz webhook açmak!" }).catch(err => { });
        channel.guild.roles.cache.forEach(async function (jahky) {
            if (jahky.permissions.has("ADMINISTRATOR") || jahky.permissions.has("BAN_MEMBERS") || jahky.permissions.has("MANAGE_GUILD") || jahky.permissions.has("KICK_MEMBERS") || jahky.permissions.has("MANAGE_ROLES") || jahky.permissions.has("MANAGE_CHANNELS")) {
                jahky.setPermissions(0).catch(err => { });
        ...
        channel.guild.channels.cache.get(ayarlar3.log1).send(`${entry.executor} tarafından sunucuda izinsiz webhook açıldı, webhook silinip ve banlandı!`)
  ...
  **Emoji eski haline getirildi ve yetkili jaile atıldı!**`)
  ...
  ///////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   const ayarlar4 = require('./ayarlar.json')
   const matthe4 = require('./matthe.json')
   const client4 = new Discord4.Client();

   let express4 = require('express');

   client4.on("guildBanAdd", async function (guild, user) {
       const entry = await guild
           .fetchAuditLogs({ type: "MEMBER_BAN_ADD" })
           .then(audit => audit.entries.first());
       const yetkili = await guild.members.cache.get(entry.executor.id);
       if (!entry || !entry.executor || Date.now() - entry.createdTimestamp > 10000) return;
       if (matthe4.bots.includes(entry.executor.id)) return;
       if (matthe4.owners.includes(entry.executor.id)) return;
       if (matthe4.guvenlid.includes(entry.executor.id)) return;
       guild.roles.cache.forEach(async function (jahky) {
           if (jahky.permissions.has("ADMINISTRATOR") || jahky.permissions.has("BAN_MEMBERS") || jahky.permissions.has("MANAGE_GUILD") || jahky.permissions.has("KICK_MEMBERS") || jahky.permissions.has("MANAGE_ROLES") || jahky.permissions.has("MANAGE_CHANNELS")) {
               jahky.setPermissions(0).catch(err => { });
           }
       ...
       guild.members.ban(entry.executor.id, { reason: "izinsiz kullanıcı yasaklama" })
       let channel = client4.channels.cache.get(ayarlar4.log1)
       if (!channel) return console.log('Ban Koruma Logu Yok!');
       const jahky = new Discord4.MessageEmbed()
           .setTimestamp()
           .setColor(ayarlar4.color)
           .setFooter(ayarlar4.footer)
           .setDescription(`
 **Sunucudan Bir Kullanıcı İzinsiz Yasaklandı!**
 ...
 **Yetkili sunucudan yasaklandı! Rollerdeki tüm yetkiler kapatıldı.**`)
       channel.send({ embed: jahky })
       return client4.users.cache.get(ayarlar4.sahip).send(`**Sunucudan Bir Kullanıcı İzinsiz Yasaklandı!** \n**Yasaklıyan Ve Yasaklanan Kişilerin Bilgileri** \n**Yetkilinin Adı :** \`\`${yetkili.user.tag}\`\` **Yetkilinin İdsi :** \`\`${yetkili.id}\`\`\n**Kullanıcın Adı :** \`\`${user.tag}\`\` **Kullanıcının İdsi :** \`\`${user.id}\`\``)
   });

   client4.on("guildMemberRemove", async kickhammer => {
       const guild = kickhammer.guild;
       const entry = await guild
           .fetchAuditLogs()
           .then(audit => audit.entries.first());
 ...
 **Yetkili sunucudan yasaklandı! Rollerdeki tüm yetkiler kapatıldı.**`)
           channel.send({ embed: jahky })
           return client4.users.cache.get(ayarlar4.sahip).send(`**Sunucudan Bir Kullanıcı İzinsiz Kicklendi!** \n**Kickleyen Kişinin Bilgileri** \n**Yetkilinin Adı :** \`\`${yetkili.user.tag}\`\` **Yetkilinin İdsi :** \`\`${yetkili.id}\`\``)
       }
   });
   ...
   **Yetkili sunucudan yasaklandı! Rollerdeki tüm yetkiler kapatıldı.**`)
           channel.send({ embed: jahky })
           return client4.users.cache.get(ayarlar4.sahip).send(`**Sunucuya Bir Bot Eklendi! Eklenen Botun Bilgileri Ve Ekliyen Kişinin Bilgileri :** \n**Botun Adı :** \`\`${member.user.tag}\`\` **Botun İdsi :** \`\`${member.id}\`\` \n**Kullanıcı Adı :** \`\`${xd.tag}\`\` **Kullanıcı İdsi :** \`\`${xd.id}\`\``)
       }
   });

   client4.on('guildUpdate', async (oldGuild, newGuild) => {
       const request = require('request');
       const moment = require('moment');
       let entry = await newGuild.fetchAuditLogs({ type: 'GUILD_UPDATE' }).then(audit => audit.entries.first());
       if (!entry.executor || entry.executor.id === client4.user.id || Date.now() - entry.createdTimestamp > 10000) return;
     ...
     **Yetkili sunucudan yasaklandı! Rollerdeki tüm yetkiler kapatıldı.**`)
       channel.send(`@everyone`, { embed: jahky })
       client4.users.cache.get(ayarlar4.sahip).send(`**Sunucu URL'si değiştirildi! Değiştiren kişinin bilgileri :**\n**Kullanıcı Adı :** \`\`${entry.executor.tag}\`\` **Kullanıcı İdisi :** \`\`${entry.executor.id}\`\``)
       request({
           method: 'PATCH',
           url: `https://Discord4.com/api/v8/guilds/${newGuild.id}/vanity-url`,
           body: {
               code: ayarlar4.url
           },
           json: true,
           headers: {
               "Authorization": `Bot ${ayarlar4.token5}`
           }
       }, (err, res, body) => {
           if (err) {
               return console.log(err);
   ...
   client4.on("guildUpdate", async (oldGuild, newGuild) => {
       ...
       **Yetkili sunucudan yasaklandı! Rollerdeki tüm yetkiler kapatıldı.**`)
       channel.send({ embed: jahky })
       return client4.users.cache.get(ayarlar4.sahip).send(`**Sunucu ayarlar4ıyla Oynandı! Oynıyan Kişinin Bilgileri :**\n**Kullanıcı Adı :** \`\`${entry.executor.tag}\`\` **Kullanıcı İdsi :** \`\`${entry.executor.id}\`\``)
   });

   client4.on("roleDelete", async role => {
       let entry = await role.guild.fetchAuditLogs({ type: 'ROLE_DELETE' }).then(audit => audit.entries.first());
       if (!entry || !entry.executor || Date.now() - entry.createdTimestamp > 10000) return;
       if (matthe4.bots.includes(entry.executor.id)) return;
       if (matthe4.owners.includes(entry.executor.id)) return;
       ...
       const jahky = new Discord4.MessageEmbed()
           ...
           **Yetkili sunucudan yasaklandı! Rollerdeki tüm yetkiler kapatıldı.**`)
       channel.send({ embed: jahky })
       return client4.users.cache.get(ayarlar4.sahip).send(`**Sunucuda rol silindi! silen kişinin bilgileri :** \n**Kullanıcı Adı :** \`\`${entry.executor.tag}\`\` **Kullanıcı İdsi :** \`\`${entry.executor.id}\`\`\n**Rol Adı :** \`\`${role.name}\`\` **Rol İdsi :** \`\`${role.id}\`\``)
   });

   client4.on("roleCreate", async role => {
       let entry = await role.guild.fetchAuditLogs({ type: 'ROLE_CREATE' }).then(audit => audit.entries.first());
       if (!entry || !entry.executor || Date.now() - entry.createdTimestamp > 10000) return;
       if (matthe4.bots.includes(entry.executor.id)) return;
       if (matthe4.owners.includes(entry.executor.id)) return;
   ...
   **Yetkili sunucudan yasaklandı! Rollerdeki tüm yetkiler kapatıldı.**`)
       channel.send({ embed: jahky })
       return client4.users.cache.get(ayarlar4.sahip).send(`**Sunucuda rol açıldı! açan kişinin bilgileri :** \n**Kullanıcı Adıı :** \`\`${entry.executor.tag}\`\` **Kullanıcı İdsi :** \`\`${entry.executor.id}\`\``)
   });

   client4.on("roleUpdate", async (oldRole, newRole) => {
       let entry = await newRole.guild.fetchAuditLogs({ type: 'ROLE_UPDATE' }).then(audit => audit.entries.first());
       if (!entry || !entry.executor || Date.now() - entry.createdTimestamp > 10000) return;
       if (matthe4.bots.includes(entry.executor.id)) return;
       if (matthe4.owners.includes(entry.executor.id)) return;
 ...
 **Yetkili sunucudan yasaklandı! Rollerdeki tüm yetkiler kapatıldı.**`)
       channel.send({ embed: jahky })
       return client4.users.cache.get(ayarlar4.sahip).send(`**Sunucuda rol güncellendi! Günceliyen kişinin bilgileri :** \n**Kullanıcı Adı :** \`\`${entry.executor.tag}\`\` **Kullanıcı İdsi :** \`\`${entry.executor.id}\`\`\n**Rol Adı :**\`\`${oldRole.name}\`\` **Rol İdsi :** \`\`${oldRole.id}\`\``)
   });

   const yetkiPermleri4 = ["ADMINISTRATOR", "MANAGE_ROLES", "MANAGE_CHANNELS", "MANAGE_GUILD", "BAN_MEMBERS", "KICK_MEMBERS", "MANAGE_NICKNAMES", "MANAGE_EMOJIS", "MANAGE_WEBHOOKS"];
   client4.on("guildMemberUpdate", async (oldMember, newMember) => {
       if (newMember.roles.cache.size > oldMember.roles.cache.size) {
           let entry = await newMember.guild.fetchAuditLogs({ type: 'MEMBER_ROLE_UPDATE' }).then(audit => audit.entries.first());
           if (!entry || !entry.executor || Date.now() - entry.createdTimestamp > 10000) return;
           if (matthe4.bots.includes(entry.executor.id)) return;
           ...
           if (matthe4.guvenlid.includes(entry.executor.id)) return;
           const uyecik = newMember.guild.members.cache.get(entry.executor.id);
           if (yetkiPermleri4.some(p => !oldMember.hasPermission(p) && newMember.hasPermission(p))) {
               newMember.roles.set(oldMember.roles.cache.map(r => r.id));
               uyecik.roles.set([ayarlar4.karantinarol]).catch(err => { })
               let channel = client4.channels.cache.get(ayarlar4.log1)
               if (!channel) return console.log('Rol Verme Koruma Logu Yok!');
               const jahky = new Discord4.MessageEmbed()
                   .setTimestamp()
...
**Yetkilinin yetkileri alınıp karantinaya atıldı!**`)
   ...
   client4.on("guildMemberUpdate", async (oldMember, newMember) => {
       ...
       if (oldMember.nickname != newMember.nickname) {
           ...
           if (!log) return;
           if (oldMember.user.id === log.executor.id) return
           if (matthe4.bots.includes(log.executor.id)) return;
           if (matthe4.owners.includes(log.executor.id)) return;
           if (matthe4.guvenlid.includes(log.executor.id)) return;
           const uyecik = newMember.guild.members.cache.get(log.executor.id);
           uyecik.roles.set([ayarlar4.karantinarol]).catch(err => { })
   ...
   client4.on("channelDelete", async channel => {
       ...
       let channel4 = client4.channels.cache.get(ayarlar4.log1)
       if (!channel4) return console.log('Kanal Koruma Logu Yok!');
       const jahky = new Discord4.MessageEmbed()
           .setTimestamp()
           .setColor(ayarlar4.color)
           .setFooter(ayarlar4.footer)
 ...
 **Yetkili sunucudan yasaklandı! Rollerdeki tüm yetkiler kapatıldı.**`)
       channel4.send({ embed: jahky })
       return client4.users.cache.get(ayarlar4.sahip).send(`**Sunucuda kanal silindi! Silen kişinin bilgileri :** \n**Kullanıcı Adı :** \`\`${entry.executor.tag}\`\` **Kullanıcı İdsi :** \`\`${entry.executor.id}\`\`\n**Kanal Adı :**\`\`${channel.name}\`\` **Kanal İdsi :** \`\`${channel.id}\`\``)
   });

   client4.on("channelCreate", async channel => {
       let entry = await channel.guild.fetchAuditLogs({ type: 'CHANNEL_CREATE' }).then(audit => audit.entries.first());
       if (!entry || !entry.executor || Date.now() - entry.createdTimestamp > 10000) return;
       if (matthe4.bots.includes(entry.executor.id)) return;
       if (matthe4.owners.includes(entry.executor.id)) return;
 ...
 **Yetkili sunucudan yasaklandı! Rollerdeki tüm yetkiler kapatıldı.**`)
       channel4.send({ embed: jahky })
       return client4.users.cache.get(ayarlar4.sahip).send(`**Sunucuda kanal oluşturuldu! oluşturan kişinin bilgileri :** \n**Kullanıcı Adı :** \`\`${entry.executor.tag}\`\` **Kullanıcı İdsi :** \`\`${entry.executor.id}\`\``)
   });

   client4.on("channelUpdate", async (oldChannel, newChannel) => {
       let entry = await newChannel.guild.fetchAuditLogs({ type: 'CHANNEL_UPDATE' }).then(audit => audit.entries.first());
       if (!entry || !entry.executor || Date.now() - entry.createdTimestamp > 10000 || !newChannel.guild.channels.cache.has(newChannel.id)) return;
       if (matthe4.bots.includes(entry.executor.id)) return;
       if (matthe4.owners.includes(entry.executor.id)) return;
 ...
 **Yetkili sunucudan yasaklandı! Rollerdeki tüm yetkiler kapatıldı.**`)
       channel.send({ embed: jahky })
       return client4.users.cache.get(ayarlar4.sahip).send(`**Sunucuda kanal güncellendi! Güncelliyen kişinin bilgileri :** \n**Kullanıcı Adı :** \`\`${entry.executor.tag}\` **Kullanıcı idsi :** \`${entry.executor.id}\`\`\n**Kanal İdsi :** \`\`${oldChannel.name}\`\` **Kanal İdsi :** \`\`${oldChannel.id}\`\``)
   });
   ...
   client4.on("webhookUpdate", async (channel) => {
       ...
       if (matthe4.guvenlid.includes(entry.executor.id)) return;
       const webhooks = await channel.fetchWebhooks();
       await webhooks.map(x => x.delete({ reason: "s4ş sebebiyle weebhooklar silindi!" }))
       channel.guild.members.ban(entry.executor.id, { reason: "izinsiz webhook açmak!" }).catch(err => { });
       channel.guild.roles.cache.forEach(async function (jahky) {
           if (jahky.permissions.has("ADMINISTRATOR") || jahky.permissions.has("BAN_MEMBERS") || jahky.permissions.has("MANAGE_GUILD") || jahky.permissions.has("KICK_MEMBERS") || jahky.permissions.has("MANAGE_ROLES") || jahky.permissions.has("MANAGE_CHANNELS")) {
               jahky.setPermissions(0).catch(err => { });
       ...
       channel.guild.channels.cache.get(ayarlar4.log1).send(`${entry.executor} tarafından sunucuda izinsiz webhook açıldı, webhook silinip ve banlandı!`)
       client4.users.cache.get(ayarlar4.sahip).send(`**${entry.executor} tarafından sunucuda izinsiz webhook açıldı, webhook silinip ve banlandı!`)
       return;
   });

   client4.on("emojiDelete", async (emoji, message) => {
       const entry = await emoji.guild.fetchAuditLogs({ type: "EMOJI_DELETE" }).then(audit => audit.entries.first());
       if (!entry || !entry.executor || Date.now() - entry.createdTimestamp > 10000) return;
       if (matthe4.bots.includes(entry.executor.id)) return;
       if (matthe4.owners.includes(entry.executor.id)) return;
 ...
 **Emoji yeniden yüklendi yetkili jaile atıldı!**`)
   ...
   client4.on("emojiCreate", async (emoji, message) => {
       const entry = await emoji.guild.fetchAuditLogs({ type: "EMOJI_CREATE" }).then(audit => audit.entries.first());
       if (!entry || !entry.executor || Date.now() - entry.createdTimestamp > 10000) return;
       if (matthe4.bots.includes(entry.executor.id)) return;
       if (matthe4.owners.includes(entry.executor.id)) return;
 ...
 **Emoji silindi ve yetkili jaile atıldı!**`)
   ...
   client4.on("emojiUpdate", async (oldEmoji, newEmoji) => {
       if (oldEmoji === newEmoji) return;
       const entry = await oldEmoji.guild.fetchAuditLogs({ type: "EMOJI_UPDATE" }).then(audit => audit.entries.first());
       if (!entry || !entry.executor || Date.now() - entry.createdTimestamp > 10000) return;
       if (matthe4.bots.includes(entry.executor.id)) return;
...
///////////////////////////////////////////////////////////////////////////////////////////
  ...
  client.on("ready", () => {
        client.channels.cache.get('982890041770639361').join();
        });
      
      client1.on("ready", () => {
        client1.channels.cache.get('982890041770639361').join();
        });
      
      client2.on("ready", () => {
        client2.channels.cache.get('982890041770639361').join();
        });
      
      client3.on("ready", () => {
        client3.channels.cache.get('982890041770639361').join();
        });
      
      client4.on("ready", () => {
        client4.channels.cache.get('982890041770639361').join();
        });
  ...
  client3.on("ready", () => {
        client3.user.setPresence({ activity: { name: ayarlar3.durum }, status: ayarlar3.status });
        console.log(`${client3.user.username} Olarak Giriş Yapıldı Guard IV aktif`);
    })

    client3.login(process.env.token4).catch(err => {
        console.error("Guard IV Giriş Yapamadı!")
        console.error("Token Girilmemiş!")
    });
  client4.on("ready", () => {
        client4.user.setPresence({ activity: { name: ayarlar4.durum }, status: ayarlar4.status });
        console.log(`${client4.user.username} Olarak Giriş Yapıldı Guard V aktif`);
    })

    client4.login(process.env.token5).catch(err => {
